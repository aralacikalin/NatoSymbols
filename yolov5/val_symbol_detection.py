...
def PlotImg(det,path,save_dir,names,correctness):
        p=path
        im0=cv2.imread(path)
        # if np.max(im0) <= 1:
...
def run(
    ...
    iouv = torch.linspace(0.8, 0.95, 4, device=device)  # iou vector for mAP@0.8:0.95
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        with dt[2]:
            ...
            allThresholdPreds = non_max_suppression(preds,
                                        default_conf_thres,
                                        iou_thres,
                                        labels=lb,
                                        multi_label=True,
        ...
        for si, pred in enumerate(preds):
            ...
            if save_txt:
                ...
                PlotImg(predn,path,save_dir,names,correct[:,0])
            if save_json:
    ...
    if len(stats) and stats[0].any():
        ...
        pAtTarget,rAtTarget= p_r_at(*stats, plot=plots, save_dir=save_dir, names=names)
