...
def ap_per_class_with_confidence(tp, conf, pred_cls, target_cls, plot=False, save_dir='.', names=(), eps=1e-16, prefix=''):
    """ Compute the average precision, given the recall and precision curves.
    Source: https://github.com/rafaelpadilla/Object-Detection-Metrics.
    # Arguments
        tp:  True positives (nparray, nx1 or nx10).
...
def p_r_at(tp, conf, pred_cls, target_cls, plot=False, save_dir='.', names=(), eps=1e-16, prefix='',targetAt=0.8):
    """ Compute the average precision, given the recall and precision curves.
    Source: https://github.com/rafaelpadilla/Object-Detection-Metrics.
    # Arguments
        tp:  True positives (nparray, nx1 or nx10).
    ...
    i = smooth(f1.mean(0), 0.1).argmax()  # max F1 index
    pi=np.where(p>=targetAt)
    ri=np.where(r>=targetAt)
    confP=px[pi[1]]
    confR=px[ri[1]]

    pAtTargetRecall=0
    rAtTargetPrecision=0
    if(np.any(ri)):
        candiditeR=r[ri]
        canditeP=p[ri]
        sortedIndexes=canditeP.argsort()
        pAtTargetRecall=canditeP[sortedIndexes[-1]]
        confR=confR[sortedIndexes[-1]]
    if(np.any(pi)):
        candiditeR=r[pi]
        canditeP=p[pi]
        sortedIndexes=candiditeR.argsort()
        rAtTargetPrecision=candiditeR[sortedIndexes[-1]]
        confP=confP[sortedIndexes[-1]]

    
    return pAtTargetRecall,rAtTargetPrecision,confP,confR
